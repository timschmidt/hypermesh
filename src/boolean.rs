...
fn arrangement_attempt_materialized_outputs_match(
    ...
    let same_source_output = retained.operation == replay.operation
        ...
        && retained.output_facts == replay.output_facts;
...
fn replay_regularized_arrangement_attempt(
    ...
    if shortcut_facts.certified_support(request.operation)
        ...
        && orthogonal_solid_cell_materializes_for_preflight(left, right, request.operation)?
        && let Some(attempt) = arrangement_cell_complex_shortcut_attempt_with_facts(
            left,
            right,
            request,
            policy,
...
mod tests {
    ...
    fn arrangement_coplanar_evidence_retains_source_handoff() {
        ...
        assert!(
            coplanar_boundary_only_evidence_if_consumed(
                &boundary_graph,
                &boundary_left,
                &stale_right,
            )
    ...
    fn selected_region_winding_evidence_classifies_retained_graph_blocker() {
        ...
        stale.blocker.kind = ExactBooleanBlockerKind::Winding;
        assert_eq!(
            stale.validate(),
        ...
        relabeled_empty.blocker.kind = ExactBooleanBlockerKind::BoundaryPolicy;
        assert_eq!(
    ...
    fn graph_empty_containment_routes_named_booleans_through_arrangement_pipeline() {
        ...
        let container =
            concatenate_meshes(&outer, &disjoint_shell, ExactMeshValidationPolicy::CLOSED)
