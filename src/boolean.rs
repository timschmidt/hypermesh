...
impl ExactBooleanRequest {
    ...
    pub fn adjacent_union_completion_report(
        ...
        Ok(adjacent_union_completion_certification(left, right, self.operation, None)?.0)
    }
    ...
    pub fn identical_mesh_report(
        ...
        identical_mesh_report_from_sources(left, right)
    }
...
pub struct ExactBooleanCertificationSet {
    ...
    /// Retained graph/split/region bridge into arrangement topology.
    pub topology_assembly: Option<ExactTopologyAssemblyReport>,
    /// Arrangement/cell ownership evidence derived from exact face labels and
    /// volume regions.
    pub region_ownership: Option<ExactRegionOwnershipReport>,
...
impl ExactBooleanCertificationSet {
    ...
    pub(crate) fn from_graph_and_regularized_arrangement(
        ...
        let direct_arrangement_cell_complex_attempt_report =
        ...
        let direct_arrangement_cell_complex_certified =
            direct_arrangement_cell_complex_attempt_report.is_some()
        ...
        let regularized_arrangement =
            if matches!(request.operation, ExactBooleanOperation::SelectedRegions(_))
                || direct_arrangement_cell_complex_certified
            {
                None
        ...
        let arrangement_attempt = if direct_arrangement_cell_complex_certified {
            direct_arrangement_cell_complex_attempt_report
        } else {
    ...
    pub fn validate_for_request(
        ...
        if let Some(region_ownership) = self.region_ownership.as_ref() {
            region_ownership
                .validate()
                .map_err(|_| ExactReportValidationError::StatusEvidenceMismatch)?;
        }
        if let Some(topology_assembly) = self.topology_assembly.as_ref() {
            topology_assembly
                .validate()
                .map_err(|_| ExactReportValidationError::StatusEvidenceMismatch)?;
        }
        ...
        if matches!(request.operation, ExactBooleanOperation::SelectedRegions(_)) {
            if self.topology_assembly.is_some()
                ...
                || self.volumetric_boundary_closure.is_some()
                || self.arrangement_attempt.is_some()
            {
                return Err(ExactReportValidationError::StatusEvidenceMismatch);
            }
            return Ok(());
        ...
        if self.topology_assembly.is_none() {
            return Err(ExactReportValidationError::StatusEvidenceMismatch);
        }
...
fn region_ownership_report_error(blocker: ExactArrangementBlocker) -> MeshError {
...
pub(crate) fn evaluate_boolean_exact_request_with_artifacts_and_arrangement_replay(
    ...
    if preflight.operation != request.operation {
        return Err(MeshError::one(MeshDiagnostic::new(
            Severity::Error,
            DiagnosticKind::UnsupportedExactOperation,
            "exact boolean evaluation preflight operation does not match request",
        )));
    }
    preflight.validate().map_err(|error| {
        MeshError::one(MeshDiagnostic::new(
            Severity::Error,
            DiagnosticKind::UnsupportedExactOperation,
            format!("exact boolean evaluation preflight validation failed: {error:?}"),
        ))
    })?;
    validate_graph_source_handoff(graph, left, right)?;
    let owned_regularized_arrangement;
...
pub(crate) fn materialize_certified_boolean_support_with_artifacts(
    ...
    let result = match support {
        ...
        | ExactBooleanSupport::CertifiedClosedBoundaryTouchingDifference => {
            ...
            if result.is_some() {
                result
            } else if let Some(graph) = retained_graph {
                materialize_closed_no_volume_overlap_regularized_boolean_with_evidence_from_graph(
                    graph, left, right, operation, validation,
                )?
                .map(|(result, _)| result)
...
fn materialize_certified_arrangement_cell_complex_support_with_arrangement(
    ...
    let no_volume_overlap = if let Some(graph) = retained_graph {
        materialize_closed_no_volume_overlap_regularized_boolean_with_evidence_from_graph(
            graph, left, right, operation, validation,
        )?
        .map(|(result, _evidence)| result)
...
fn direct_arrangement_cell_complex_result(
    left: &ExactMesh,
    right: &ExactMesh,
    operation: ExactBooleanOperation,
    validation: ValidationPolicy,
...
pub(crate) fn direct_arrangement_cell_complex_attempt(
    left: &ExactMesh,
    right: &ExactMesh,
    request: ExactBooleanRequest,
    policy: ExactRegularizationPolicy,
    ...
    let Some(result) =
        direct_arrangement_cell_complex_result(left, right, request.operation, request.validation)?
    else {
        return Ok(None);
    };
...
fn same_surface_report_from_sources(left: &ExactMesh, right: &ExactMesh) -> ExactSameSurfaceReport {
...
/// triangle order.
fn identical_mesh_report_from_sources(
    left: &ExactMesh,
    right: &ExactMesh,
) -> ExactIdenticalMeshReport {
...
mod tests {
    ...
    fn selected_overlay_faces_triangulate_simple_coplanar_difference_cells() {
        ...
        let readiness = ExactBooleanRequest::with_boundary_policy(
            ExactBooleanOperation::Difference,
            ValidationPolicy::ALLOW_BOUNDARY,
            ExactBoundaryBooleanPolicy::Reject,
        )
    ...
    fn selected_region_winding_readiness_classifies_retained_graph_blocker() {
        ...
        .unwrap();
        let disjoint_readiness = ExactBooleanRequest::with_boundary_policy(
            ExactBooleanOperation::SelectedRegions(ExactRegionSelection::KeepAll),
            ValidationPolicy::ALLOW_BOUNDARY,
            ExactBoundaryBooleanPolicy::Reject,
        )
    ...
    fn arrangement_result_retains_consumed_topology_and_ownership_reports() {
        ...
        let outcome = materialized_arrangement_attempt_outcome(&mut attempt, result, false);
        ...
        let report_attempt = ExactBooleanRequest::new(
            ExactBooleanOperation::Union,
            ValidationPolicy::ALLOW_BOUNDARY,
        )
        .arrangement_attempt(&left, &right, ExactRegularizationPolicy::REGULARIZED_SOLID)
    ...
    fn nonorthogonal_closed_boundary_touching_shortcuts_report_provenance() {
        ...
        assert!(
            ...
            .boundary_touching_report(&left, &overlapping_right)
            .unwrap()
            .is_certified()
        );
        assert!(
