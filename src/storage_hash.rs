...
#[cfg(test)]
mod tests {
    use super::StorageIdentityHasher;

    #[test]
    fn identity_hash_preserves_full_word() {
        use std::hash::Hasher;

        let mut first = StorageIdentityHasher::default();
        first.write_usize(0x1000);
        let mut second = StorageIdentityHasher::default();
        second.write_usize(0x2000);
