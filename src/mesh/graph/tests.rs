...
#[test]
fn endpoint_touch_split_point_reports_missing_and_parameters() {
    let left_start = p(0, 0, 0);
    let left_end = p(4, 0, 0);
    let right_start = p(6, 0, 0);
    let right_end = p(8, 0, 0);
    assert_eq!(
        endpoint_touch_split_point(
            [&left_start, &left_end],
            [&right_start, &right_end],
            CoplanarProjection::Xy,
        ),
    ...
    let touching_start = p(2, 0, 0);
    let touching_end = p(2, 2, 0);
    let split = endpoint_touch_split_point(
        [&left_start, &left_end],
        [&touching_start, &touching_end],
        CoplanarProjection::Xy,
    )
    .unwrap();
    assert_eq!(split.point, touching_start);
    assert_eq!(split.left_parameter, q(1, 2));
    assert_eq!(split.right_parameter, Real::from(0));
}
