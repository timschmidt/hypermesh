...
impl ExactBooleanCertificationSet {
    ...
    pub(crate) fn validate_for_request(
        ...
        if request.validation == MeshValidationMode::CLOSED && coplanar_closure_matches_request {
        ...
        let arrangement_shortcut_supported = self
            .arrangement_cell_complex_shortcuts
            .materializes_operation(request.operation);
        let arrangement_shortcut_materialized_for_request =
        ...
        let arrangement_output_materialized_for_request =
            if let Some(attempt) = self.arrangement_attempt.as_ref() {
                attempt
                    .validate_for_request_regularization_mode(
                        request,
        ...
        let retained_attempt_resolves_request =
            if let Some(attempt) = self.arrangement_attempt.as_ref() {
                attempt.retained_gate_reports().is_some()
                    ...
                    && attempt.mode == ExactRegularizationMode::REGULARIZED_SOLID
    ...
    fn arrangement_attempt_matches_certified_operation_evidence(
        &self,
        operation_evidence: &ExactBooleanOperationEvidence,
    ) -> bool {
        self.winding_evidence.status
            == ExactWindingEvidenceStatus::ArrangementCellComplexAlreadyMaterialized
            && self.arrangement_attempt_certifies_output_for_operation(operation_evidence.operation)
    }

    fn boundary_report_matches_operation_evidence(
        &self,
        operation_evidence: &ExactBooleanOperationEvidence,
        requires_blocker: bool,
    ) -> bool {
        operation_evidence.graph_had_unknowns == self.boundary_touching.graph_had_unknowns
            && operation_evidence.retained_face_pairs == self.boundary_touching.retained_face_pairs
            && operation_evidence.retained_events == self.boundary_touching.retained_events
            && operation_evidence.region_count == 0
            && operation_evidence.region_classifications.is_empty()
            && operation_evidence.coplanar_arrangement_evidence.is_none()
            && operation_evidence.coplanar_volumetric_evidence.is_none()
            && if requires_blocker {
                operation_evidence.blocker.as_ref() == Some(&self.boundary_touching.blocker)
            } else {
                matches!(&operation_evidence.blocker, None)
            }
    }

    fn closed_boundary_touching_matches_operation_evidence(
        &self,
        operation_evidence: &ExactBooleanOperationEvidence,
    ) -> bool {
        self.closed_boundary_touching_materialization_certified_by_retained_evidence()
            && ((self.winding_evidence.status
                == ExactWindingEvidenceStatus::ClosedBoundaryTouchingAlreadyMaterialized
                && (self.boundary_report_matches_operation_evidence(operation_evidence, false)
                    || (operation_evidence.graph_had_unknowns
                        == self.winding_evidence.graph_had_unknowns
                        && operation_evidence.retained_face_pairs
                            == self.winding_evidence.retained_face_pairs
                        && operation_evidence.retained_events
                            == self.winding_evidence.retained_events
                        && operation_evidence.region_count == self.winding_evidence.region_count
                        && operation_evidence.region_classifications
                            == self.winding_evidence.region_classifications.as_slice()
                        && matches!(&operation_evidence.blocker, None)
                        && operation_evidence.coplanar_arrangement_evidence.is_none()
                        && operation_evidence.coplanar_volumetric_evidence.is_some()
                        && operation_evidence.coplanar_volumetric_evidence.as_ref()
                            == self.winding_evidence.coplanar_volumetric_evidence.as_ref())))
                || self
                    .arrangement_attempt_matches_certified_operation_evidence(operation_evidence))
    }
    ...
    fn open_surface_arrangement_matches_operation_evidence(
        &self,
        operation_evidence: &ExactBooleanOperationEvidence,
    ) -> bool {
        (self.winding_evidence.status
            == ExactWindingEvidenceStatus::OpenSurfaceArrangementAlreadyMaterialized
            && operation_evidence.graph_had_unknowns == self.winding_evidence.graph_had_unknowns
            && operation_evidence.retained_face_pairs == self.winding_evidence.retained_face_pairs
            && operation_evidence.retained_events == self.winding_evidence.retained_events
            && operation_evidence.region_count == self.winding_evidence.region_count
            && operation_evidence.region_classifications
                == self.winding_evidence.region_classifications.as_slice()
            && matches!(&operation_evidence.blocker, None)
            ...
            && self
                ...
                .as_ref()
                .is_none())
            || (self.arrangement_attempt_matches_certified_operation_evidence(operation_evidence)
                && operation_evidence.graph_had_unknowns == self.refinement.graph_had_unknowns
                && operation_evidence.retained_face_pairs == self.refinement.retained_face_pairs
                && operation_evidence.retained_events == self.refinement.retained_events
                && operation_evidence.region_count != 0
                && !operation_evidence.region_classifications.is_empty()
                && matches!(&operation_evidence.blocker, None)
                && operation_evidence.coplanar_arrangement_evidence.is_none()
    ...
    fn winding_evidence_matches_operation_evidence(
        &self,
        operation_evidence: &ExactBooleanOperationEvidence,
    ) -> bool {
        operation_evidence.graph_had_unknowns == self.winding_evidence.graph_had_unknowns
            ...
            && operation_evidence.region_classifications
                == self.winding_evidence.region_classifications.as_slice()
            && operation_evidence.blocker.as_ref() == Some(&self.winding_evidence.blocker)
            && operation_evidence.coplanar_arrangement_evidence.is_none()
            && operation_evidence.coplanar_volumetric_evidence.as_ref()
                == self.winding_evidence.coplanar_volumetric_evidence.as_ref()
    }

    fn convex_reports_match_operation_evidence_support(
        &self,
        operation_evidence: &ExactBooleanOperationEvidence,
    ) -> bool {
        ...
        match operation_evidence.support {
            ...
            | ExactBooleanSupport::CertifiedConvexDifference => {
                match operation_evidence.operation {
                    ExactBooleanOperation::Union => self.convex_capabilities.can_union,
                    ExactBooleanOperation::Intersection => {
                        self.convex_capabilities.can_intersection
                    }
                    ExactBooleanOperation::Difference => self.convex_capabilities.can_difference,
                    ExactBooleanOperation::SelectedRegions(_) => false,
                }
    ...
    fn arrangement_cell_complex_matches_operation_evidence(
        &self,
        operation_evidence: &ExactBooleanOperationEvidence,
    ) -> bool {
        let coplanar_boundary_only_evidence_matches = operation_evidence.graph_had_unknowns
            == self.winding_evidence.graph_had_unknowns
            && operation_evidence.retained_face_pairs == self.winding_evidence.retained_face_pairs
            && operation_evidence.retained_events == self.winding_evidence.retained_events
            && operation_evidence.region_count == self.winding_evidence.region_count
        ...
        let coplanar_boundary_closure_evidence_matches = operation_evidence.graph_had_unknowns
            == self.winding_evidence.graph_had_unknowns
            && operation_evidence.retained_face_pairs == self.winding_evidence.retained_face_pairs
            && operation_evidence.retained_events == self.winding_evidence.retained_events
            && matches!(&operation_evidence.blocker, None)
        ...
        let source_fact_materialization_retains_operation_evidence_evidence = operation_evidence
            .graph_had_unknowns
            == self.winding_evidence.graph_had_unknowns
            && operation_evidence.retained_face_pairs == self.winding_evidence.retained_face_pairs
            && operation_evidence.retained_events == self.winding_evidence.retained_events
            ...
            && operation_evidence.region_classifications
                == self.winding_evidence.region_classifications.as_slice()
            && operation_evidence.coplanar_arrangement_evidence.as_ref()
                == self.winding_evidence.coplanar_arrangement_evidence.as_ref()
            && operation_evidence.coplanar_volumetric_evidence.as_ref()
                == self.winding_evidence.coplanar_volumetric_evidence.as_ref();
        let source_fact_materialization_collapsed_winding_evidence = !operation_evidence
            .graph_had_unknowns
            && !self.winding_evidence.graph_had_unknowns
        ...
        let source_fact_materialization_evidence_matches = (self
            .arrangement_cell_complex_shortcuts
            .materializes_operation(operation_evidence.operation)
            || self.winding_evidence.status
                == ExactWindingEvidenceStatus::ArrangementCellComplexAlreadyMaterialized)
            && matches!(
                self.winding_evidence.status,
            ...
            && matches!(&operation_evidence.blocker, None)
            && (source_fact_materialization_retains_operation_evidence_evidence
                || source_fact_materialization_collapsed_winding_evidence);
        let retained_attempt_evidence_matches = operation_evidence.graph_had_unknowns
            ...
            && ((self
                ...
                || self.arrangement_attempt_certifies_output_for_operation(
                    operation_evidence.operation,
                ));
        retained_attempt_evidence_matches
            || (matches!(
                ...
                ExactAdjacentUnionCompletionStatus::CertifiedFullFace
                    | ExactAdjacentUnionCompletionStatus::CertifiedContainedFace
            ) && self.adjacent_union_completion.operation == operation_evidence.operation
                && operation_evidence.operation == ExactBooleanOperation::Union
                && operation_evidence.graph_had_unknowns
                    == self.adjacent_union_completion.graph_had_unknowns
                && operation_evidence.retained_face_pairs
                    == self.adjacent_union_completion.retained_face_pairs
                && operation_evidence.retained_events
                    == self.adjacent_union_completion.retained_events
                && operation_evidence.region_count == 0
                && operation_evidence.region_classifications.is_empty()
                && matches!(&operation_evidence.blocker, None)
                && operation_evidence.coplanar_arrangement_evidence.is_none())
            ...
            || (if let Some(attempt) = self.arrangement_attempt.as_ref() {
                attempt.retained_gate_reports().is_some()
                    && attempt.operation == operation_evidence.operation
                    && attempt.mode == ExactRegularizationMode::REGULARIZED_SOLID
                    && attempt.resolves_requested_volume_ownership()
                    && {
                        let region_evidence_matches = (operation_evidence.region_count
    ...
    fn matches_operation_evidence(
        ...
        match operation_evidence.support {
            ...
            | ExactBooleanSupport::CertifiedOpenSurfaceArrangementDifference => {
                self.open_surface_arrangement_matches_operation_evidence(operation_evidence)
            }
            ExactBooleanSupport::CertifiedArrangementCellComplex => {
                self.arrangement_cell_complex_matches_operation_evidence(operation_evidence)
            }
            ExactBooleanSupport::CertifiedEmptyOperand => {
                (self.winding_evidence.status
                    == ExactWindingEvidenceStatus::EmptyOperandAlreadyMaterialized
                    || self.arrangement_attempt_matches_certified_operation_evidence(
                        operation_evidence,
                    ))
                    && (self.trivial.left_empty || self.trivial.right_empty)
            }
            ...
            | ExactBooleanSupport::CertifiedClosedBoundaryTouchingDifference => {
                self.closed_boundary_touching_matches_operation_evidence(operation_evidence)
            }
            ExactBooleanSupport::CertifiedOpenSurfaceDisjoint => {
                (self.winding_evidence.status
                    ...
                    && operation_evidence.graph_had_unknowns
                        == self.open_surface_disjoint.graph_had_unknowns
                    && operation_evidence.retained_face_pairs
                        == self.open_surface_disjoint.retained_face_pairs
                    && operation_evidence.retained_events
                        == self.open_surface_disjoint.retained_events
                    && operation_evidence.region_count == 0
                    && operation_evidence.region_classifications.is_empty()
                    && matches!(&operation_evidence.blocker, None)
                    && operation_evidence.coplanar_arrangement_evidence.is_none()
            ...
            | ExactBooleanSupport::CertifiedConvexDifference => {
                (self.winding_evidence.status
                    ...
                    && self.convex_reports_match_operation_evidence_support(operation_evidence)
            }
            ExactBooleanSupport::CertifiedConvexSeparated => {
                (self.winding_evidence.status
                    == ExactWindingEvidenceStatus::ConvexBooleanAlreadyMaterialized
            ...
            ExactBooleanSupport::RequiresBoundaryOnlyContact => {
                self.boundary_touching.status == ExactBoundaryTouchingStatus::Certified
                    && self.winding_evidence.status
                        == ExactWindingEvidenceStatus::BoundaryOnlyContactRequired
                    && self.boundary_report_matches_operation_evidence(operation_evidence, true)
            }
            ExactBooleanSupport::RequiresPlanarArrangement => {
                self.winding_evidence.status
                    == ExactWindingEvidenceStatus::PlanarArrangementRequired
                    && operation_evidence.graph_had_unknowns
                        == self.planar_arrangement.graph_had_unknowns
                    && operation_evidence.retained_face_pairs
                        == self.planar_arrangement.retained_face_pairs
                    && operation_evidence.retained_events == self.planar_arrangement.retained_events
                    && operation_evidence.region_count == 0
                    && operation_evidence.region_classifications.is_empty()
                    && operation_evidence.blocker.as_ref() == Some(&self.planar_arrangement.blocker)
                    && operation_evidence.coplanar_arrangement_evidence.as_ref()
                        == self
                            .planar_arrangement
                            .coplanar_arrangement_evidence
            ...
            ExactBooleanSupport::RequiresCoplanarVolumetricCells => {
                self.winding_evidence.status
                    ...
                    && self.winding_evidence_matches_operation_evidence(operation_evidence)
            }
...
impl ExactBooleanEvaluation {
    pub(crate) fn validate_with_optional_result(
        ...
        if self.operation_evidence.operation != self.request.operation {
            return Err(ExactEvidenceValidationError::StatusEvidenceMismatch);
        }
        self.operation_evidence.validate()?;
        self.certifications.validate_for_request(self.request)?;
        if !self
            .certifications
            .matches_operation_evidence(&self.operation_evidence)
        {
            return Err(ExactEvidenceValidationError::StatusEvidenceMismatch);
        }
        if let Some(result) = self.result.as_ref() {
            if !self.operation_evidence.support.is_certified()
            ...
            && self.operation_evidence.support.is_certified()
            && matches!(&self.operation_evidence.blocker, None)
            && !matches!(
    ...
    /// and the materialized result under the original request mode.
    pub(crate) fn validate_against_sources(
        ...
        if self.operation_evidence != replay.operation_evidence {
        ...
        if self.certifications != replay.certifications
            && (!self
                ...
                || !replay
                    .certifications
                    .matches_operation_evidence(&replay.operation_evidence))
        {
            return Err(ExactEvidenceValidationError::SourceReplayMismatch);
        }
        if let Some(result) = self.result.as_ref() {
        ...
        } else if self.operation_evidence.support.is_certified()
            && matches!(&self.operation_evidence.blocker, None)
            && !matches!(
                self.operation_evidence.support,
                ExactBooleanSupport::SelectedRegionAssembly
    ...
    fn validate_materialized_result(
        ...
        if match result.kind {
            ...
            | ExactBooleanResultKind::OpenSurfaceArrangement { .. } => {
                result.graph_had_unknowns != self.operation_evidence.graph_had_unknowns
                    || result.region_classifications
                        != self.operation_evidence.region_classifications
            }
        ...
        if result_matches_operation_evidence_support {
            Ok(())
        } else {
            Err(ExactEvidenceValidationError::StatusEvidenceMismatch)
        }
...
fn test_arrangement_attempt(
    ...
    assert_eq!(mode, ExactRegularizationMode::REGULARIZED_SOLID);
    let graph = build_validated_intersection_graph(left, right).unwrap();
...
fn assert_current_arrangement_attempt(
    ...
    let request = ExactBooleanRequest {
        ...
        validation: attempt.output_validation,
    };
    attempt
...
fn exact_mesh_shape_accepts_same_boundary_with_different_triangulation() {
    let diagonal = Mesh::from_i64_triangles_with_validation_mode(
        &[0, 0, 0, 4, 0, 0, 4, 4, 0, 0, 4, 0],
        &[0, 1, 2, 0, 2, 3],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
    .unwrap();
    let centered = Mesh::from_i64_triangles_with_validation_mode(
...
fn open_surface_disjoint_graph_shortcut_replays_sources_before_acceptance() {
    ...
    .unwrap();
    let overlapping_right = Mesh::from_i64_triangles_with_validation_mode(
        &[0, 0, 0, 4, 0, 4, 0, 4, 0],
        &[0, 1, 2],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
...
fn named_materialization_rejects_stale_retained_graph_before_ready_graph_use() {
    ...
    .unwrap();
    let crossing_right = Mesh::from_i64_triangles_with_validation_mode(
        &[1, -1, -1, 1, 3, 1, 1, 3, -1],
        &[0, 1, 2],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
...
#[test]
fn open_surface_disjoint_operation_evidence_prefers_specific_support_over_cell_complex() {
    let left = Mesh::from_i64_triangles_with_validation_mode(
        &[0, 0, 0, 4, 0, 0, 0, 4, 0],
        &[0, 1, 2],
        MeshValidationMode::ALLOW_BOUNDARY,
    ...
    let operation_evidence = test_operation_evidence(request, &left, &right);
    assert_eq!(
    ...
    assert!(
        operation_evidence.blocker.is_none(),
        "{operation_evidence:?}"
    );
    ...
    with_test_evaluation(request, &left, &right, |evaluation| {
        assert_eq!(
            evaluation.operation_evidence.support,
            ExactBooleanSupport::CertifiedOpenSurfaceDisjoint,
            "{evaluation:?}"
        );
        ...
        assert!(
            matches!(materialized.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == ExactBooleanOperation::Union && result_shortcut == ExactBooleanShortcutKind::OpenSurfaceDisjoint),
            "{materialized:?}"
        );
...
fn coplanar_volumetric_gate_uses_source_side_evidence() {
    ...
    assert!(boundary_graph.face_pairs.iter().any(|pair| matches!(
        pair.relation,
...
#[test]
fn arrangement_coplanar_evidence_retains_source_boundary() {
    let boundary_left = axis_aligned_box_i64([0, 0, 0], [2, 2, 2]);
    let boundary_right = axis_aligned_box_i64([2, 0, 0], [4, 2, 2]);
    let boundary_graph =
...
fn disconnected_contained_face_adjacent_union_replays_result_source() {
    ...
    let (mut report, result) = adjacent_union_completion_certification_from_graph(
        ...
        &right,
        ExactBooleanOperation::Union,
        Some(MeshValidationMode::CLOSED),
    )
    .unwrap();
    assert_eq!(
        report.status,
...
fn assert_contained_face_adjacent_union_replays(
    ...
    assert!(
        matches!(result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == ExactBooleanOperation::Union && result_shortcut == ExactBooleanShortcutKind::ArrangementCellComplex
        )
    );
    result.validate_against_sources(left, right).unwrap();
}
...
fn selected_overlay_faces_triangulate_simple_coplanar_difference_cells() {
    ...
    assert!(overlay.blockers.is_empty());
...
fn projected_face_ring_rejects_stale_face_rows() {
    let mesh = Mesh::from_i64_triangles_with_validation_mode(
        &[0, 0, 0, 4, 0, 0, 4, 4, 0, 0, 4, 0],
        &[0, 1, 2, 0, 2, 3],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
...
fn selected_region_winding_evidence_classifies_retained_graph_blocker() {
    ...
    let disjoint_right = Mesh::from_i64_triangles_with_validation_mode(
    ...
    relabeled_empty.blocker = relabeled_blocker;
    assert_eq!(
...
fn selected_overlay_faces_absorb_contained_union_components() {
    let outer_square = Mesh::from_i64_triangles_with_validation_mode(
        &[0, 0, 0, 4, 0, 0, 4, 4, 0, 0, 4, 0],
        &[0, 1, 2, 0, 2, 3],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
    .unwrap();
    let inner_square = Mesh::from_i64_triangles_with_validation_mode(
...
fn projected_overlay_mesh_uses_certified_output_components() {
    let ring = |region, points: &[(i64, i64)]| ExactArrangement2dRegionRing {
        region,
        vertices: points
            .iter()
    ...
    let projection = choose_nonzero_projected_polygon_area(&carrier_points).unwrap();
...
fn coplanar_overlay_certifies_component_holed_contact_difference() {
    ...
    .unwrap();
    let opening_plus_hole = Mesh::from_i64_triangles_with_validation_mode(
        &[
    ...
    let operation_evidence = test_operation_evidence(request, &left, &opening_plus_hole);
    assert_eq!(
        operation_evidence.support,
        ExactBooleanSupport::CertifiedArrangementCellComplex,
        "{operation_evidence:?}"
    ...
    assert!(
        matches!(result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == ExactBooleanOperation::Difference && result_shortcut == ExactBooleanShortcutKind::ArrangementCellComplex
        )
    );
}
...
fn coplanar_overlay_materializes_point_touching_hole_difference() {
    ...
    .unwrap();
    let touching_holes = Mesh::from_i64_triangles_with_validation_mode(
        &[
            1, 1, 0, 3, 1, 0, 3, 3, 0, 1, 3, 0, //
            3, 3, 0, 5, 3, 0, 5, 5, 0, 3, 5, 0,
        ],
...
fn coplanar_overlay_materializes_containment_union_and_intersection() {
    let outer_triangle = Mesh::from_i64_triangles_with_validation_mode(
        &[0, 0, 0, 4, 0, 0, 0, 4, 0],
        &[0, 1, 2],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
    .unwrap();
    let inner_triangle = Mesh::from_i64_triangles_with_validation_mode(
        &[1, 1, 0, 2, 1, 0, 1, 2, 0],
        &[0, 1, 2],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
...
fn arrangement_preempts_multi_triangle_coplanar_overlay_including_containment() {
    ...
    let inner = Mesh::from_i64_triangles_with_validation_mode(
        &[1, 1, 0, 2, 1, 0, 1, 2, 0],
        &[0, 1, 2],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
    ...
    assert!(
        matches!(union.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == ExactBooleanOperation::Union && result_shortcut == ExactBooleanShortcutKind::ArrangementCellComplex
        )
    );
}

#[test]
fn materialized_arrangement_operation_evidence_probe_certifies_full_pipeline_output() {
    ...
    let operation_evidence = certified_arrangement_cell_complex_evidence_if_materialized(
        ExactBooleanOperation::Union,
        &graph,
        &left,
        &right,
    ...
    assert!(operation_evidence.blocker.is_none());
    assert_eq!(
        operation_evidence.retained_face_pairs,
        graph.face_pairs.len()
    );
    assert_eq!(operation_evidence.retained_events, graph.event_count());

    let materialized = materialize_axis_aligned_orthogonal_solid_cell_output(
        &left,
        &right,
        AxisAlignedOrthogonalSolidOperation::Union,
        "test axis-aligned box arrangement operation evidence materialization",
        request.validation,
    )
    .unwrap()
    .expect("operation evidence support should have an exact orthogonal cell output");
...
fn certifications_reuse_regularized_arrangement_attempt_reports() {
    ...
    assert!(attempt.materialized_shortcut.is_none(), "{attempt:?}");
...
fn axis_aligned_orthogonal_cell_booleans_materialize_from_shortcut_support() {
    ...
    for operation in [
        ...
        let mut stale_operation_evidence = operation_evidence.clone();
        stale_operation_evidence
            .coplanar_volumetric_evidence
            .as_mut()
            .expect("operation evidence should retain coplanar volumetric evidence")
            .retained_face_pair_count = operation_evidence.retained_face_pairs + 1;
        assert!(
            stale_operation_evidence.validate().is_err(),
            "{operation:?}: {stale_operation_evidence:?}"
        );
        ...
        assert!(
            matches!(result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == operation && result_shortcut == ExactBooleanShortcutKind::ArrangementCellComplex
            ),
            "{operation:?}: {result:?}"
        );
        result
...
fn axis_aligned_orthogonal_union_reaches_generic_arrangement_triangulation() {
    ...
    let labeled = arrangement.label_regions(labeling_mode).unwrap();
...
#[test]
fn affine_box_booleans_materialize_from_certified_operation_evidence_support() {
...
fn affine_empty_intersection_materializes_without_winding_fallback() {
    ...
    assert!(
        matches!(result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == ExactBooleanOperation::Intersection && result_shortcut == ExactBooleanShortcutKind::ArrangementCellComplex
        )
    );
    result.validate_against_sources(&left, &right).unwrap();
...
fn trivial_shortcuts_report_materialized_evidence() {
    let empty = empty_mesh("empty operand evidence fixture", MeshValidationMode::CLOSED).unwrap();
    let solid = axis_aligned_box_i64([0, 0, 0], [2, 2, 2]);
    let far_solid = axis_aligned_box_i64([4, 0, 0], [6, 2, 2]);
    let left_open = Mesh::from_i64_triangles_with_validation_mode(
        &[
            0, 0, 0, //
            4, 0, 4, //
            0, 4, 0,
    ...
    .unwrap();
    let right_open = Mesh::from_i64_triangles_with_validation_mode(
        &[
            0, 0, 1, //
            4, 0, 5, //
            0, 4, 1,
    ...
    for (left, right, validation, support, status, shortcut) in [
        ...
        for operation in [
            ...
            let operation_evidence = test_operation_evidence(request, left, right);
            assert_eq!(
                operation_evidence.support, support,
                "{operation:?}: {operation_evidence:?}"
            );
            assert!(
                operation_evidence.blocker.is_none(),
            ...
            assert!(
                matches!(result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == operation && result_shortcut == shortcut),
                "{operation:?}: {result:?}"
            );
            result.validate().unwrap();
            result.validate_against_sources(left, right).unwrap();
    ...
    for (left, right, right_inside_left) in [
        ...
        for operation in [
            ...
            assert_eq!(
                operation_evidence.support,
                ExactBooleanSupport::CertifiedArrangementCellComplex,
                "{right_inside_left:?} {operation:?}: {operation_evidence:?}"
            );
            assert!(
                operation_evidence.blocker.is_none(),
                "{operation:?}: {operation_evidence:?}"
...
fn graph_empty_closed_winding_separation_materializes_without_bounds_disjointness() {
    ...
    for operation in [
        ...
        assert!(
            matches!(result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == operation && result_shortcut == ExactBooleanShortcutKind::ClosedWindingSeparated),
            "{operation:?}: {result:?}"
        );
        result.validate().unwrap();
        result.validate_against_sources(&left, &right).unwrap();
...
fn mixed_dimensional_regularized_solid_reports_materialized_evidence() {
    ...
    let sheet = Mesh::from_i64_triangles_with_validation_mode(
        &[1, 1, 1, 3, 1, 1, 1, 3, 1],
        &[0, 1, 2],
        MeshValidationMode::ALLOW_BOUNDARY,
    )
    ...
    for (left, right) in [(&solid, &sheet), (&sheet, &solid)] {
        for operation in [
            ...
            let operation_evidence = test_operation_evidence(
                ExactBooleanRequest {
                    operation: operation,
                    validation: MeshValidationMode::ALLOW_BOUNDARY,
                },
            ...
            assert!(
                matches!(result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == operation && result_shortcut == ExactBooleanShortcutKind::MixedDimensionalRegularizedSolid),
                "{operation:?}: {result:?}"
            );
            result.validate().unwrap();
            result.validate_against_sources(left, right).unwrap();
...
#[test]
fn closed_operation_evidence_does_not_certify_boundary_only_arrangement_output() {
    ...
    let operation_evidence = test_operation_evidence(reject_closed_request, &left, &right);
    assert_eq!(
        operation_evidence.support,
        ExactBooleanSupport::RequiresCertifiedWinding,
        "{operation_evidence:?}"
    ...
    assert!(
        operation_evidence.blocker.is_some(),
        "{operation_evidence:?}"
    );
    operation_evidence.validate().unwrap();
    operation_evidence
    ...
    let boundary_operation_evidence = test_operation_evidence(
        ExactBooleanRequest {
            operation: ExactBooleanOperation::Union,
            validation: MeshValidationMode::ALLOW_BOUNDARY,
        },
    ...
    assert_eq!(
        boundary_operation_evidence.support,
        ExactBooleanSupport::CertifiedArrangementCellComplex,
        "{boundary_operation_evidence:?}"
    );
    assert!(
        boundary_operation_evidence.blocker.is_none(),
        "{boundary_operation_evidence:?}"
    );
    assert_eq!(
        boundary_operation_evidence.retained_face_pairs,
        graph.face_pairs.len()
    );
    assert_eq!(
        boundary_operation_evidence.retained_events,
        graph.event_count()
    );
    boundary_operation_evidence.validate().unwrap();
    boundary_operation_evidence
    ...
    assert!(
        boundary_operation_evidence
            .validate_against_sources_for_request(&left, &right, reject_closed_request)
            .is_err(),
        "closed replay should not certify an allow-boundary operation evidence"
    );
...
fn arrangement_attempt_certified_as_cell_complex_for_operation_evidence_with_validation(
    left: &Mesh,
    right: &Mesh,
    operation: ExactBooleanOperation,
...
#[test]
fn arrangement_operation_evidence_probe_keeps_boundary_valid_open_output_separate() {
    let left = Mesh::from_i64_triangles_with_validation_mode(
        &[0, 0, 0, 4, 0, 0, 0, 4, 0],
        &[0, 1, 2],
        MeshValidationMode::ALLOW_BOUNDARY,
    ...
    for operation in [
        ...
        assert!(
            !arrangement_attempt_certified_as_cell_complex_for_operation_evidence_with_validation(
                &left,
                &right,
                operation,
                MeshValidationMode::CLOSED
...
fn arrangement_result_retains_consumed_topology_and_ownership_reports() {
    ...
    assert_eq!(
        retained_attempt.stage,
        ExactArrangementBooleanStage::Materialized
    );
    assert!(retained_attempt.decline.is_none());
    assert!(retained_attempt.materialized_shortcut.is_none());
    assert!(
        retained_attempt
            .validate_for_request_regularization_mode(
                request,
            ...
            .is_ok()
            && retained_attempt.materialized_arrangement_cell_complex_output()
    );
    ...
    assert!(
        report_attempt
            ...
            .is_ok()
            && report_attempt.materialized_arrangement_cell_complex_output()
    );
    assert!(
        retained_attempt.operation == ExactBooleanOperation::Union
            && retained_attempt.mode == ExactRegularizationMode::REGULARIZED_SOLID
            && retained_attempt.validate().is_ok()
            && retained_attempt.materialized_arrangement_cell_complex_output()
    );
    let mut relabeled_attempt = retained_attempt.clone();
    relabeled_attempt.operation = ExactBooleanOperation::Difference;
    assert!(
        !(relabeled_attempt.operation == ExactBooleanOperation::Union
            && relabeled_attempt.mode == ExactRegularizationMode::REGULARIZED_SOLID
            && relabeled_attempt.validate().is_ok()
            && relabeled_attempt.materialized_arrangement_cell_complex_output())
    );
    let mut wrong_validation_attempt = report_attempt.clone();
    wrong_validation_attempt.output_validation = MeshValidationMode::CLOSED;
    assert!(
        !(wrong_validation_attempt
            ...
            .is_ok()
            && wrong_validation_attempt.materialized_arrangement_cell_complex_output())
    );
    assert_eq!(
        wrong_validation_attempt.validate_for_request_regularization_mode(
            ExactBooleanRequest {
                operation: ExactBooleanOperation::Union,
                validation: MeshValidationMode::ALLOW_BOUNDARY,
            },
...
#[test]
fn retained_volume_ownership_operation_evidence_rejects_stale_source_replay() {
    let left = tetrahedron_i64([0, 0, 0], [10, 0, 0], [0, 10, 0], [0, 0, 10]);
    let right = tetrahedron_i64([1, 1, 1], [2, 1, 1], [1, 2, 1], [1, 1, 2]);
    let request = ExactBooleanRequest {
        operation: ExactBooleanOperation::Difference,
    ...
    let graph = build_validated_intersection_graph(&left, &right).unwrap();
    let error = certified_arrangement_cell_complex_evidence_from_retained_attempt(
        &graph,
        &left,
        &right,
        request,
    ...
    .expect_err("source-stale retained ownership must not certify operation evidence");
    assert!(
...
fn crossing_open_surface_boolean_materializes_inside_arrangement_attempt() {
    ...
    for operation in [
        ...
        let operation_evidence = test_operation_evidence(request, &left, &right);
        assert_eq!(
            operation_evidence.support, expected_support,
            "{operation:?}: {operation_evidence:?}"
        );
        assert!(
            operation_evidence.blocker.is_none(),
        ...
        assert!(
            operation_evidence.validate().is_ok(),
            "{operation:?}: {operation_evidence:?}"
        );
        operation_evidence
            .validate_against_sources_for_request(&left, &right, request)
        ...
        assert_eq!(evidence.region_count, operation_evidence.region_count);
        assert_eq!(
            evidence.region_classifications.as_slice(),
            operation_evidence.region_classifications.as_slice()
        );
        evidence.validate().unwrap();
        evidence.validate_against_sources(&left, &right).unwrap();
        ...
        let is_open_surface_arrangement = matches!(
            result.kind,
            ExactBooleanResultKind::OpenSurfaceArrangement {
                operation: result_operation,
            } if result_operation == operation
        ...
        result.validate().unwrap();
        result.validate_against_sources(&left, &right).unwrap();
        if is_open_surface_arrangement {
...
fn partial_face_boundary_touch_is_regularized_without_coplanar_cell_blocker() {
    ...
    let intersection = test_operation_evidence(intersection_request, &left, &right);
    assert_eq!(
    ...
    let difference = test_operation_evidence(difference_request, &left, &right);
    assert_eq!(
        difference.support,
        ExactBooleanSupport::CertifiedArrangementCellComplex
    );
    ...
    assert!(
        matches!(intersection.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == ExactBooleanOperation::Intersection && result_shortcut == ExactBooleanShortcutKind::ArrangementCellComplex
        )
    );
    ...
    assert!(
        matches!(difference.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == ExactBooleanOperation::Difference && result_shortcut == ExactBooleanShortcutKind::ArrangementCellComplex
        )
    );
...
fn nested_closed_shell_booleans_materialize_through_arrangement_pipeline() {
    ...
    for operation in [
        ...
        assert_eq!(
            operation_evidence.retained_face_pairs, 0,
            "{operation:?}: {operation_evidence:?}"
        );
        ...
        assert!(
            matches!(result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == operation && result_shortcut == expected_shortcut)
                || matches!(
                        result.kind,
                        ExactBooleanResultKind::ArrangementCellComplexMaterialized {
                            operation: result_operation,
...
fn nonorthogonal_closed_boundary_touching_shortcuts_report_provenance() {
    ...
    assert_ne!(
        test_boundary_touching_report(&left, &overlapping_right).status,
        ExactBoundaryTouchingStatus::Certified
    );
    assert!(
    ...
    assert!(
        ...
        "overlapping volume should not be classified as boundary-only mode evidence"
    );
...
fn boundary_attached_contained_tetrahedron_difference_materializes() {
    ...
    assert!(
        operation_evidence.retained_events > 0,
        "{operation_evidence:?}"
    );
    let mut relabeled_operation_evidence = operation_evidence.clone();
    relabeled_operation_evidence.support = ExactBooleanSupport::CertifiedConvexDifference;
    assert!(relabeled_operation_evidence.validate().is_err());
    let evidence = test_winding_evidence(
        ExactBooleanRequest {
            operation: ExactBooleanOperation::Difference,
            validation: MeshValidationMode::ALLOW_BOUNDARY,
...
fn straddling_coplanar_crossing_tetrahedron_boundary_attempt_materializes() {
    ...
    for operation in [
        ...
        if expected_support == ExactBooleanSupport::CertifiedArrangementCellComplex {
            ...
            assert!(
                matches!(retained_result.kind, ExactBooleanResultKind::CertifiedShortcut { operation: result_operation, shortcut: result_shortcut } if result_operation == operation && result_shortcut == ExactBooleanShortcutKind::ArrangementCellComplex
                ),
            ...
            assert!(match dispatched_result.kind {
                ExactBooleanResultKind::SelectedRegions { selection } => {
                ...
                | ExactBooleanResultKind::ArrangementCellComplexMaterialized {
                    operation: result_operation,
                } => result_operation == request.operation,
            });
            assert!(
                dispatched_result
                    .mesh
...
fn selected_region_assembly_reports_stale_triangulation_artifacts() {
    ...
    .unwrap();
    let right =
        Mesh::from_i64_triangles_with_validation_mode(&[], &[], MeshValidationMode::ALLOW_BOUNDARY)
            .unwrap();
    let boundary = vec![
...
fn exact_coplanar_boundary_canonicalizes_only_degenerate_self_contact_spurs() {
    ...
    let exact_equal = |left: &Point3, right: &Point3| {
    ...
    let nondegenerate_self_contact =
        canonicalize_degenerate_cyclic_self_contact(vec![a.clone(), b, c, a, d, e], &exact_equal)
    ...
    let split = split_cyclic_self_contact_cycles(nondegenerate_self_contact, &exact_equal)
...
fn closed_identical_solids_route_through_arrangement_pipeline() {
    ...
    assert!(attempt.materialized_shortcut.is_none());
    assert_current_arrangement_attempt(&attempt, &left, &right);
...
fn closed_same_surface_solids_route_through_arrangement_pipeline() {
    ...
    assert_eq!(
        same_surface_report_from_sources(&left, &right).status,
...
fn arrangement_materialized_evidence_retains_boundary_only_evidence() {
    ...
    let left = Mesh::new_with_validation_mode_and_version(
        vertices,
        triangles,
    ...
    assert!(
        matches!(
            ...
            CoplanarVolumetricCellObstacle::BoundaryOnlyContact
        ) && volumetric_evidence.positive_area_coplanar_overlapping_pairs != 0
    );
    assert_eq!(
